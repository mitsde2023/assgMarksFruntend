...
const Subject = () => {
    ...
    useEffect(() => {
        axios.get('http://65.0.91.129:7000/api/student/subject-class')
            .then(response => setSubjectClassData(response.data))
            .catch(error => console.error('Error fetching data:', error.message));
    }, []);
    
    const handleUpdate = (subjectId, subjectName) => {
        const updateOne = axios.post(`http://65.0.91.129:7000/api/marks/subject_marks_update/${subjectId}/${subjectName}`);
